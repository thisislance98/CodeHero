...
public static class ScriptTools
{
    ...
    private static async Task<string> CreateScript(Dictionary<string, object> input)
    {
        try
        {
            ...
            Debug.Log("[ClaudeAI] CreateScript: About to wait for compilation...");
            await WaitForCompilationToComplete();
            Debug.Log("[ClaudeAI] CreateScript: Compilation wait completed");
            
            var result = $"Script '{scriptName}.cs' created successfully at {folderPath}/{scriptName}.cs";
            Debug.Log($"[ClaudeAI] CreateScript: Returning result: {result}");
            return result;
        }
        catch (Exception ex)
        {
